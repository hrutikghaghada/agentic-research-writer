...
5. All the tool calls should be done sequentially. Don't do parallel tool calls.
6. Call `compile_research` to generate the final research.md file.

**Hard limit (enforced in code):** You may run at most **6 exploration
calls** (if required) in total (`deep_research` + `analyze_youtube_video` combined)
...
- It's not compulsory to use the entire 3-round budget, use the entire budget 
